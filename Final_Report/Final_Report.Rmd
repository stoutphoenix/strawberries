...
```{r}
production_data <-read.csv("Cleaned_datasets/relevant_strawberries_wide.csv")
shipment_data <- read.csv("Cleaned_datasets/quarterly_vols_strawberries.csv")
...
```{r cars, echo=FALSE}
pacman::p_load(readr)
df <- read_csv("Cleaned_datasets/combined_data.csv")
```
...
```{r}
df <- read.csv('Cleaned_datasets/combined_data.csv')
library(ggplot2)
library(dplyr)
...
This analysis successfully achieved its objective by sourcing, processing, and integrating two disparate USDA datasets—NASS production statistics and AMS refrigerated shipment data—to build a unified model of the U.S. strawberry market.

The data processing and cleaning revealed a critical and prescriptive insight that guided the entire analysis: the U.S. strawberry production and refrigerated transportation networks are, in effect, a two-state system. The visualizations in Section 4 demonstrated that California and Florida account for nearly 100% of all relevant activity, rendering the combined production and shipment volumes of all "Other States" statistically insignificant.

By focusing on these two key states, the analysis uncovered two fundamentally different market structures:

**California: The High-Volume Production Powerhouse**

-   California dominates the market in sheer volume, operating at a scale that dwarfs all other states.

-   This high volume corresponds to a lower average price compared to Florida.

-   Critically, California's shipment-to-production ratio remains stable and consistently below 1.0 (typically 0.6–0.8). This, combined with a shipment-to-production slope of just 0.507, provides strong evidence that a significant portion of its massive output is diverted to channels other than fresh refrigerated transport, such as processing, freezing, or alternative distribution.

**Florida: The High-Price, Logistically-Volatile Hub**

-   Florida operates as a lower-volume, higher-price market, commanding a significant premium per CWT.

-   Its market behavior is highly volatile. Most notably, its shipment-to-production ratio frequently exceeds 1.0, peaking at 1.2 in 2017.

-   This finding implies that Florida's refrigerated shipment data does not just represent its own production. It strongly suggests Florida acts as a key logistical hub, handling and shipping inventory from previous periods or (more likely) imported strawberries to meet market demand, especially during its peak season.

In summary, the U.S. strawberry supply chain is not a single entity but a tale of two distinct models. California behaves as a stable, diversified producer, while Florida acts as a fast-moving, high-stakes, and logistically complex hub. This foundational work provides a clear, data-driven framework for any future analysis of transportation logistics, market timing, or economic factors within this industry.
